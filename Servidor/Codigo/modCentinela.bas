...
'############################################################
    ...
    If Userindex = Centinela.RevisandoUserIndex Then
        If Not UserList(Userindex).flags.CentinelaOK Then
            Call SendData(SendTarget.toindex, Userindex, 0, "N|" & vbWhite & "°" & "¡La clave que te he dicho es " & "/CENTINELA " & Centinela.clave & " escríbelo rápido!" & "°" & CStr(Npclist(CentinelaNPCIndex).Char.CharIndex))
        Else
            'Logueamos el evento
            Call LogCentinela("El usuario " & UserList(Centinela.RevisandoUserIndex).name & " respondió más de una vez la contraseña correcta.")
            Call SendData(SendTarget.toindex, Userindex, 0, "N|" & vbWhite & "°" & "Te agradezco, pero ya me has respondido. Me retiraré pronto." & "°" & CStr(Npclist(CentinelaNPCIndex).Char.CharIndex))
        End If
    Else
        Call SendData(SendTarget.toindex, Userindex, 0, "N|" & vbWhite & "°" & "No es a ti a quien estoy hablando, ¿no ves?" & "°" & CStr(Npclist(CentinelaNPCIndex).Char.CharIndex))
    End If
End Sub
...
'############################################################
    ...
    Else
        ...
        Else
            ...
            Call SendData(SendTarget.toindex, Centinela.RevisandoUserIndex, 0, "N|" & vbRed & "°¡" & UserList(Centinela.RevisandoUserIndex).name & ", tienes un minuto más para responder! Debes escribir /CENTINELA " & Centinela.clave & "." & "°" & CStr(Npclist(CentinelaNPCIndex).Char.CharIndex))
        End If
    End If
End Sub

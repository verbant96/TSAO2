    ...
    If Npclist(NpcIndex).flags.Inmovilizado = 0 Or Npclist(NpcIndex).Char.Heading = headingloop Then
        ...
        If InMapBounds(nPos.Map, nPos.X, nPos.Y) Then
            ...
            If UI > 0 And Not atacoPJ Then
                If UserList(UI).flags.Muerto = 0 Then
                    atacoPJ = True
                    If Npclist(NpcIndex).flags.LanzaSpells <> 0 And MapData(nPos.Map, nPos.X + 1, nPos.Y).userindex <> 0 And MapData(nPos.Map, nPos.X - 1, nPos.Y).userindex <> 0 And MapData(nPos.Map, nPos.X, nPos.Y + 1).userindex <> 0 And MapData(nPos.Map, nPos.X, nPos.Y - 1).userindex <> 0 Then
                    ...
                    If Npclist(NpcIndex).TargetNPC = NI Then
                         ...
                         Else
                            ...
                            If Distancia(Npclist(NpcIndex).Pos, Npclist(NI).Pos) <= 1 Then
                                Call SistemaCombate.NpcAtacaNpc(NpcIndex, NI, False)
                            End If
                         End If
                         Exit Sub
                    End If
